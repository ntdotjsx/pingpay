...
export function CreateTripModal({
  ...
  const notAdded = searchResults.filter(
    (m) => !members.some((sel) => sel.user_id === m.id),
  );
  const alreadyAdded = searchResults.filter((m) =>
    members.some((sel) => sel.user_id === m.id),
  );
  ...
  const handleCreate = async () => {
    if (!proofUrl) {
      alert('กรุณาแนบภาพหลักฐานการโอนเงินก่อน');
      return;
    }
    setSaving(true);
    try {
      ...
      alert(`สร้างทริป "${tripName}" สำเร็จ!`);
      reset();
      onCreated();
    } catch (e: any) {
      alert(e.message ?? 'สร้างกลุ่มไม่สำเร็จ');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[90vh] max-w-sm overflow-y-auto">
        ...
        <div className="mb-1 flex items-center gap-1.5">
          {(['info', 'members', 'confirm'] as Step[]).map((s, i) => (
            <div key={s} className="flex items-center gap-1.5">
              <div
                className={`flex h-6 w-6 items-center justify-center rounded-full text-[11px] font-semibold transition-colors ${
                  step === s
                    ...
                    : i < ['info', 'members', 'confirm'].indexOf(step)
              ...
              {i < 2 && (
                <div
                  className={`h-px w-8 flex-1 ${i < ['info', 'members', 'confirm'].indexOf(step) ? 'bg-emerald-500' : 'bg-muted'}`}
                />
              )}
            </div>
          ))}
                ...
                <p className="text-center text-xs font-medium text-rose-500">
                  *{' '}
                  {!tripName.trim()
                    ? 'กรุณากรอกชื่อทริป'
                    : amount <= 0
                      ? 'กรุณาระบุจำนวนเงินต่อคนให้ถูกต้อง'
                      : !proofUrl
                        ? 'กรุณาแนบภาพหลักฐานการโอนเงิน'
                        : ''}
                </p>
              )}
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              {showDropdown && searchQ && (
                <div className="border-border bg-background mt-1 overflow-hidden rounded-lg border shadow-sm">
                  {searching && (
                    <p className="text-muted-foreground flex items-center gap-1.5 px-3 py-2 text-xs">
                  ...
                  {/* ยังไม่เพิ่ม — กดได้ */}
                  {!searching &&
                    notAdded.slice(0, 5).map((m) => (
                      <button
                        key={m.id}
                        onClick={() => addApiMember(m)}
                        className="hover:bg-muted/50 flex w-full items-center gap-2 px-3 py-2 text-left text-sm transition-colors"
                      >
                        <div className="bg-muted flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[11px] font-semibold">
                          {m.name[0]}
                        </div>
                  ...
                  {!searching &&
                    alreadyAdded.slice(0, 3).map((m) => (
                      <div
                        key={`added-${m.id}`}
                        className="flex w-full cursor-not-allowed items-center gap-2 px-3 py-2 text-sm opacity-50 select-none"
                      >
                        <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-[11px] font-semibold text-emerald-700 dark:bg-emerald-900 dark:text-emerald-300">
                          ✓
                        </div>
                        <span className="flex-1">{m.name}</span>
              ...
              <div>
                ...
                <div className="bg-muted/30 overflow-hidden rounded-xl">
                  {members.map((m) => (
                    <div
                      key={m.id}
                      className="border-border/50 flex items-center gap-2.5 border-b px-3 py-2 last:border-0"
                    >
                      <div className="bg-foreground/10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[11px] font-semibold">
                        {m.name[0]}
                      </div>
            ...
            <div className="space-y-2 pt-2">
              {members.length === 0 && (
                <p className="text-center text-xs font-medium text-rose-500">
                  * กรุณาเพิ่มสมาชิกในทริปอย่างน้อย 1 คน
                </p>
              )}
              <div className="flex gap-2">
                <Button
        ...
        {step === 'confirm' && (
          <div className="space-y-3">
            <div className="bg-muted/40 space-y-2 rounded-xl p-3.5">
            ...
            <div>
              ...
              <div className="space-y-1">
                {members.map((m) => (
                  <div
                    key={m.id}
                    className="bg-background border-border flex items-center gap-2 rounded-lg border px-3 py-2"
                  >
