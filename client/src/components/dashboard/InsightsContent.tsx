...
function Metric({
  ...
  return (
    ...
    >
      <p
        className={`text-[10px] font-medium tracking-widest uppercase ${tone === 'strong' ? 'text-background/60' : 'text-muted-foreground'}`}
      >
        {label}
      </p>
      ...
      {sub && (
        <p
          className={`mt-2 text-xs ${tone === 'strong' ? 'text-background/60' : 'text-muted-foreground'}`}
        >
          {sub}
        </p>
      )}
...
export function InsightsContent() {
  ...
  useEffect(() => {
    api
      .getCreditorInsights()
      .then(setData)
      .catch(() => console.error('โหลดสถิติไม่สำเร็จ'))
    ...
    <div className="space-y-4 p-1">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-xs">
            ภาพรวมยอดให้ยืม การคืนเงิน และรายการที่ต้องตาม
          </p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric
          ...
          sub={`${data.active_loans_count} รายการ active`}
          tone="strong"
        />
        <Metric
          label="คืนแล้ว"
          value={fmt(data.recovered)}
          sub={`${data.recovery_rate}% ของยอดให้ยืม`}
        />
        <Metric
          label="เกินกำหนด"
          value={fmt(data.overdue_amount)}
          sub="ยอดที่ควรติดตามก่อน"
          tone="warn"
        />
        <Metric
          label="รออ่านสลิป"
          value={String(data.pending_confirmations)}
          sub="รายการชำระที่ส่งเข้ามา"
        />
      </div>
        ...
        <div className="text-muted-foreground mt-3 grid gap-2 text-xs sm:grid-cols-3">
          <div className="bg-muted/35 rounded-lg p-3">
            ...
            <p className="text-foreground mt-1 font-semibold tabular-nums">
              {fmt(data.average_ticket)}
            </p>
          </div>
          <div className="bg-muted/35 rounded-lg p-3">
            <p>ปิดแล้ว</p>
            <p className="text-foreground mt-1 font-semibold tabular-nums">
              {data.settled_loans_count} รายการ
            </p>
          </div>
          <div className="bg-muted/35 rounded-lg p-3">
            <p>ยัง active</p>
            <p className="text-foreground mt-1 font-semibold tabular-nums">
              {data.active_loans_count} รายการ
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
            ...
            <div className="space-y-3">
              {data.top_debtors.map((item) => (
                <Bar
                  ...
                  label={`${item.borrower.name} (${item.loan_count})`}
        ...
        <section className="border-border/60 bg-background rounded-xl border p-4">
          ...
          {data.monthly_recovery.length ? (
            ...
            <div className="py-10 text-center">
              <p className="text-muted-foreground text-sm">
                ยังไม่มีประวัติการคืนเงินที่ยืนยันแล้ว
              </p>
            </div>
          )}
        </section>
